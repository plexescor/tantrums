...
# This scrip works on relative directories
...
def check_exists(test_file_name, test_file_expected_output):
    ...
    if not os.path.isfile(test_file_path):
        print(f"The test file '{test_file_path}' does not exists!")
        sys.exit(1)
    elif not os.path.isfile(test_file_expected_output_path):
        print(f"The test file '{test_file_expected_output_path}' does not exists!")
        sys.exit(1)
...
def run_test(test_file_name, test_file_expected_output):
    ...
    temp_result = subprocess.run(
        [tantrums_path, test_file_name, "-o", test_obj, "-l", test_exe],
...
def main():
    ...
    tantrums_path = input("Enter the relative path to your tantrum's executable: ")
    check_exists(tantrums_path, tantrums_path)
